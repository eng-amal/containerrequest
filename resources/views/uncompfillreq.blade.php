...
@extends('master')
@section('title',  __('contreq.pagename9'))
@section('content')
...
<main class="main">
      <div>
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-left mb-2">
                    <h2>{{ __('contreq.pagename9') }}</h2>
                </div>
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ url('/uncomfillind') }}"> Back</a>
                </div>
            </div>
        </div>
        @if(session('status'))
        ...
        <form action="{{ route('uncompfillreq1',$liftreq->id) }}" method="POST" enctype="multipart/form-data" class="form-group p-3 flex row">
        @csrf
        @method('POST')
                    ...
                    <div class="w-50 p-3">
                       <label>{{ __('contreq.unliftimg') }}</label>
                       <img src="{{ asset('storage/' . $liftreq->unliftimg) }}" alt="Container Image" width="200">
                    </div>
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.unliftreason') }}</strong>
                        <select id="unliftreasonid" name="unliftreasonid"  class="form-select">
                         
                         <option value="">{{ __('contreq.unliftreason') }}</option>
                          @foreach ($unliftreasons as $cont)
                          <option value="{{ $cont->id }}" {{ $liftreq->unliftreasonid  == $cont->id ? 'selected' : '' }}>
                        {{ $cont->name }}
                          </option>
                         @endforeach
                        </select>
                        
                        @error('unliftreasonid')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.fromdate') }}</strong>
                        <input type="datetime-local" name="rereqdate"  class="form-control" placeholder="rereqdate">
                        @error('rereqdate')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    </div>
