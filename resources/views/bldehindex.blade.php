@extends('master')
@section('title',  __('contreq.pagename61'))
...
<!-- Starter Section Section -->
    <section id="starter-section" class="starter-section section">
      ...
      <div class="pull-left mb-2">
        <h2 class="text-color">{{ __('contreq.pagename61') }}</h2>
      </div> 
        ...
        <a class="btn" href="{{ route('createbldeh') }}">
        ...
        <table class="table table-hover table-bordered">
            ...
            <tbody>
                @foreach ($bldehs as $bldeh)
                    <tr>
                        <td >{{ $bldeh->id }}</td>
                        <td>{{ $bldeh->name }}</td>
                        <td>{{ $bldeh->enname }}</td>
                        <td>
                            <form action="{{ route('destroybldeh',$bldeh->id) }}" method="Post">
