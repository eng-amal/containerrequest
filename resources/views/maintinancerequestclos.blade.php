@extends('master')
@section('title',  __('contreq.pagename96'))
@section('content')
...
<main class="main">
    <div>
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-left mb-2">
                    <h2>{{ __('contreq.pagename96') }}</h2>
                </div>
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ route('maintinancerequestclose') }}"> {{ __('contreq.back') }}</a>
                </div>
            </div>
        </div>
        @if(session('status'))
        ...
        @method('POST')
                    ...
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.empprocessid') }}</strong>
                        <input type="text" name="empprocessid" readonly value="{{ $maintinancerequest->empprocessid }}" class="form-control">
                    </div>
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.processnote') }}</strong>
                        <input type="text" name="processnote" readonly value="{{ $maintinancerequest->processnote }}" class="form-control">
                    </div>
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.processdate') }}</strong>
                        <input type="text" name="processdate" readonly value="{{ $maintinancerequest->processdate }}" class="form-control">
                    </div>
                    
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.closenote') }}</strong>
                        <input type="date" name="closenote" id="closenote" value="{{ old('closenote') }}" class="form-control" placeholder="closenote">
                        @error('closenote')
