...
@extends('master')
@section('title',  __('contreq.pagename33'))
  ...
  <main class="main">
    ...
    <div >
      ...
      <div >
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-left mb-2">
                    <h2 class="text-color">{{ __('contreq.pagename33') }}</h2>
                </div>
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ route('stayindex', $employee->id) }}"> {{ __('contreq.back') }}</a>
                </div>
            </div>
        </div>
        ...
        @endif
        <form action="{{ route('storestay') }}" method="POST" enctype="multipart/form-data" class="form-group p-3 flex row">
            @csrf
            <input type="hidden" name="empid" value="{{ $employee->id }}">
                    
            <div class="w-25 p-3">
                        <strong>{{ __('contreq.staynum') }}</strong>
                        <input type="text" requerid name="staynum" id="staynum" value="{{ old('staynum') }}" id="staynum" class="form-control" placeholder="{{ __('contreq.staynum') }}">
                        @error('staynum')
