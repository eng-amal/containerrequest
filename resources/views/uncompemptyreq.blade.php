...
@extends('master')
@section('title',  __('contreq.pagename11'))
@section('content')
...
<main class="main">
      <div>
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-left mb-2">
                    <h2>{{ __('contreq.pagename11') }}</h2>
                </div>
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ url('/emptyind') }}"> Back</a>
                </div>
            </div>
        </div>
        @if(session('status'))
        ...
        <form  enctype="multipart/form-data" class="form-group p-3 flex row">
        @csrf
        @method('POST')
                    ...
                    <div class="w-50 p-3">
                       <label>{{ __('contreq.aftercontimg') }}</label>
                       <img src="{{ asset('storage/' . $liftreq->contimg) }}" alt="Container Image" width="200">
                    </div>
                    <div class="w-50 p-3">
                       <label>{{ __('contreq.locimage') }}</label>
                       <img src="{{ asset('storage/' . $liftreq->locimage) }}" alt="Container Image" width="200">
                    </div>
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.emptyres') }}</strong>
                        <select id="emptyreasonid" name="emptyreasonid"  class="form-select">
                         
                         <option value="">{{ __('contreq.emptyres') }}</option>
                          @foreach ($unemptyreasons as $cont)
                          <option value="{{ $cont->id }}" {{ $liftreq->emptyreasonid   == $cont->id ? 'selected' : '' }}>
                        {{ $cont->name }}
                          </option>
                         @endforeach
                        </select>
                    ...
                    <div class="w-25 p-3">
                        <strong>{{ __('contreq.contlocation') }}</strong>
                        <select id="empcontlocationid" name="empcontlocationid"  class="form-select">
                         
                         <option value="">{{ __('contreq.contlocation') }}</option>
                          @foreach ($contlocations as $cont)
                          <option value="{{ $cont->id }}" {{ $liftreq->empcontlocationid   == $cont->id ? 'selected' : '' }}>
                        {{ $cont->name }}
                          </option>
                         @endforeach
                        </select>
                        
                        @error('empcontlocationid')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    </div>
